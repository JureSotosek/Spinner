...
class MainBackView: UIView {
    ...
    lazy var circleCenter: CGPoint = { [unowned self] in
        if self.bounds.height < 500 {
            return CGPoint(x: self.bounds.midX, y: self.bounds.height-318)
        }else if self.bounds.height < 600 {
            return CGPoint(x: self.bounds.midX, y: self.bounds.height-363)
        }else if self.bounds.height < 700{
            return CGPoint(x: self.bounds.midX, y: self.bounds.height-422)
        }else{
